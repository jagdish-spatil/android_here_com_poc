 ...
 */
public class BackgroundPositioningActivity1 extends AppCompatActivity implements PositioningManager.OnPositionChangedListener {

    private final static String TAG = BackgroundPositioningActivity1.class.getSimpleName();
    public static double tempLat = 0;
    public static double tempLong = 0;
    Timer timer;
    ...
    private static final String[] RUNTIME_PERMISSIONS1 = {
            Manifest.permission.ACCESS_FINE_LOCATION,
            Manifest.permission.WRITE_EXTERNAL_STORAGE,
            Manifest.permission.INTERNET,
            Manifest.permission.ACCESS_WIFI_STATE,
    ...
    private final static PositioningManager.OnPositionChangedListener sListener = new PositioningManager.OnPositionChangedListener() {
        ...
        public void onPositionUpdated(PositioningManager.LocationMethod locationMethod, GeoPosition geoPosition, boolean mapMatched) {
            tempLat = geoPosition.getCoordinate().getLatitude();
            tempLong = geoPosition.getCoordinate().getLongitude();
            System.out.println("APP Background State Location Method *== "+locationMethod.name()+", Latitude="+tempLat+", Longitude="+tempLong);
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (hasPermissions(this)) {
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q){
                ActivityCompat.requestPermissions(this, RUNTIME_PERMISSIONS1, REQUEST_CODE_ASK_PERMISSIONS);
            } else {
...
//    }
    ...
    private static boolean hasPermissions(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q){
                for (String permission : RUNTIME_PERMISSIONS1) {
                    if (ActivityCompat.checkSelfPermission(context, permission) != PackageManager.PERMISSION_GRANTED) {
                        return false;
                    }
                }
    ...
    private void initializeMapsAndPositioning() {
        setContentView(R.layout.act_bkg_service);
        ...
        mTogglePositioning.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                try {
                    if (isChecked) {
                        startListening(BackgroundPositioningActivity1.this);
                        ...
                        stopListening(BackgroundPositioningActivity1.this);
                        if(timer!=null){
                            timer.cancel();
                        }
                    }
    ...
    private void startForegroundService() {
        final Intent intent = new Intent(getApplicationContext(), BackgroundPositioningActivity1.class);
    ...
    private void updateLocationInfo(PositioningManager.LocationMethod locationMethod, GeoPosition geoPosition) {
        ...
        tempLat = geoPosition.getCoordinate().getLatitude();
        tempLong = geoPosition.getCoordinate().getLongitude();
        System.out.println("APP Running State Location Method *== "+locationMethod.name()+", Latitude="+tempLat+", Longitude="+tempLong);
    }
    ...
    public void uploadLatLong(){
        try{
            DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
            Date curDate = new Date();

            System.out.println("uploadLatLong called *==");
            System.out.println("uploadLatLong tempLat *=="+tempLat+", tempLong="+tempLong+", DateTime="+dateFormat.format(curDate));
...
//                }
                ...
                location.setName(sharedpreferences.getString("name","No User Exist"));
                location.setLatitude(tempLat);
                location.setLongitude(tempLong);
                location.setOs(getAndroidVersion());
                location.setDevice(getDeviceName()+", "+dateFormat.format(curDate));
                System.out.println("*== I/P location "+new Gson().toJson(location));
                mAPIService = ApiUtils.getAPIService();
                mAPIService.updateLocation(location).enqueue(new Callback<ResponseBody>() {

                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        try {
                            System.out.println("O/p response *==  " + response.body().string());
                        }catch (Exception e){
